  ...
  .then((data) => {
    ...
    categories.forEach((category) => {
      ...
      categoryThumb.className =
        "bg-white shadow-lg relative text-center cursor-pointer rounded-lg overflow-hidden hover:shadow-xl transition";
      categoryThumb.innerHTML = `
        <img src="${strCategoryThumb}" alt="${strCategory}" class="w-full h-32 object-cover p-2">
    ...
    .then((data) => {
      ...
      ingredientsContainer.innerHTML = `
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          ${mealData.strTags ? `<p class="text-gray-700 my-1"><strong>Tags:</strong> ${mealData.strTags}</p>` : ""}
          ${mealData.strSource ? `<p class="text-blue-600 mt-2"><a href="${mealData.strSource}" target="_blank" class="underline hover:text-blue-800">View Source</a></p>` : ""}
        </div>
        </div>
      `;
